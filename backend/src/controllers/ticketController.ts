...
export async function listTickets(
  ...
  if (rawSortBy !== undefined) {
    if (
      ...
      !ALLOWED_SORT_FIELDS.includes(rawSortBy as SortField)
    ) {
      throw new AppError(
...
export async function createTicket(
  ...
  if (
    !isEnumValue(TicketStatus, status) ||
    !isEnumValue(TicketPriority, priority)
  ) {
  ...
  if (!project || !creator || (assigneeId && !assignee)) {
    throw new AppError(
      "The selected project, creator, or assignee was not found.",
      400,
    );
  }
